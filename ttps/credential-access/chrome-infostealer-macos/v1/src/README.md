## Description
MacOS Chrome InfoStealer Simulation V1 is a Python script designed to simulate the extraction of sensitive information from Google Chrome on macOS systems. The script provides comprehensive functionality for credential access and system reconnaissance:
...
- `-g`, `--getSafe`: Retrieve the Chrome safe storage password
...
### Other Options
- `-t`, `--timeout`: Timeout duration in seconds (default: 10)
...
# Set timeout duration for retrieving safe storage password to 15 seconds.
python main.py --getSafe --memCopy --allProfiles --timeout=15
```
...
# Retrieve safe storage password, copy Chrome data files for all profiles to memory
# Retrieve system information, set timeout duration for retrieving safe storage password to 15 seconds, and enable verbose logging
python main.py --getSafe --memCopy --allProfiles --systemInfo --timeout=15 --verbose
```
...
- **Process Termination Logic**: The `terminate_process` function attempts to kill the SecurityAgent process by assuming it has a PID offset from the security process. This assumption might not always hold true, leading to potential failures in terminating the correct process.
- **Password Extraction**: The logic for extracting the password from stderr assumes a specific format. If the output format changes or if there are multiple password entries, this logic might fail.
