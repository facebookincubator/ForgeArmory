...
```bash
ttpforge run forgearmory//persistence/dll-side-loading/ttp.yaml
```
```bash
ttpforge run forgearmory//persistence/dll-side-loading/ttp.yaml --arg process_name=calc
```
```bash
ttpforge run forgearmory//persistence/dll-side-loading/ttp.yaml --arg gup_exe=test\myGUP.exe --arg curl_dll=test\mylibcurl.dll
```
